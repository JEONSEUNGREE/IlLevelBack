...
//    @Value("file.save.company-path")
    ...
    @Autowired
    public AppRoomServiceImpl(RoomService roomService, RoomPicService roomPicService, CompanyService companyService, ImgUtils imgUtils) {
    ...
    @Override
    public AppRoomDTO companyRoomCreate(LoginCompanyInfo loginCompanyInfo, AppRoomView appRoomView, MultipartFile thumbNailImg, List<MultipartFile> roomImgList) {
        CompanyMS companyMS = companyService.getCompanyByComEmail(loginCompanyInfo.getComEmail()).orElseThrow(NotCompanyUserException::new);
        ...
        /* 객실 이미지 이름들 추출 */
        if (roomImgList != null) {
            roomImgFileNameList = roomImgList.stream().map(item -> imgUtils.fileNameCreate(item.getOriginalFilename())).toList();
        }
        ...
        try {
            ...
            if (roomImgList != null) {
                for (int i = 0; i < roomImgList.size(); i++) {
                    imgUtils.saveAndChangeFile(roomImgList.get(i), "", roomImgFileNameList.get(i), defaultCompanyImgRoute);
                }
            }
        } catch (Exception e) {
