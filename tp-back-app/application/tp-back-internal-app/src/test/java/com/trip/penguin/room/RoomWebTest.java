...
public class RoomWebTest extends AbstractRestDocsTests {
	...
	public void roomModifyTest() throws Exception {
		...
		MockMultipartFile thumbNailImg = new MockMultipartFile("thumbNailImg", "image.png",
			MediaType.MULTIPART_FORM_DATA_VALUE, "example".getBytes());
		...
		mockMvc.perform(multipart("/room/create")
			...
			.andDo(restDocs.document(
				...
				responseFields(
					...
					fieldWithPath("data.roomPicDTOList[].picLocation").type(JsonFieldType.STRING)
						.description("객실 사진 위치 명"),
					fieldWithPath("data.roomPicDTOList[].picSeq").type(JsonFieldType.NUMBER).description("객실 사진 순서"),
					fieldWithPath("data.roomPicDTOList[].modifiedDate").type(JsonFieldType.STRING)
						.description("객실 사진 생성 일시"),
					fieldWithPath("data.roomPicDTOList[].createdDate").type(JsonFieldType.STRING)
						.description("객실 사진 수정 일시"),
	...
	@Test
	@DisplayName("객실 정보 상세")
	@WithMockCustomUser
	public void getAppRoomDetail() throws Exception {

		// given
		given(appRoomService.getRoomDetail(
			any(Long.class)))
			.willReturn(
				AppRoomDTO.builder()
					.id(1L)
		...
		// when
		mockMvc.perform(get("/room/detail/{roomId}", 1L)
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON))
			// then
			.andExpect(status().isOk())
			.andDo(restDocs.document(
				pathParameters(
					parameterWithName("roomId").description("객실 ID")
				),
				responseFields(
					fieldWithPath("message").type(JsonFieldType.STRING).description("메세지"),
					fieldWithPath("result").type(JsonFieldType.STRING).description("결과"),
